...
class SimpleVectorDB:
    def __init__(self, persist_dir: str):
        ...
        if not os.path.exists(persist_dir):
            try:
                ...
                public.print_log(f"[ERROR] Failed to create vector DB directory {persist_dir}: {str(e)}")
                raise
    ...
    def load(self):
        if os.path.exists(self.persist_file):
            try:
                ...
                public.print_log(f"[ERROR] Failed to load vector store {self.persist_file}: {str(e)}")
                self.data = []
    ...
    def save(self):
        try:
            ...
            public.print_log(f"[ERROR] Failed to save vector store {self.persist_file}: {str(e)}")
            pass
    ...
    def search(self, query_embedding: List[float], n_results: int = 5, score: float = 0.5, where: Dict = None,
...
class RAGService:
    def __init__(
        ...
        self.embedding_api_key = embedding_api_key or openai_api_key or ""
        self.embedding_base_url = embedding_base_url or openai_base_url or ""
    ...
    def add_memory(self, user_msg: Dict, ai_msg: Dict, session_id: str):
        ...
        if isinstance(user_content, list):
            user_text = "\n".join([item.get("text", "") for item in user_content if
        ...
        if isinstance(ai_content, list):
            ai_text = "\n".join(
                [item.get("text", "") for item in ai_content if isinstance(item, dict) and item.get("type") == "text"])
        else:
    ...
    def search(self, query: str, session_id: str = None, score: float = 0.2, scope: str = "session",
               exclude_ids: List[str] = None, full_text: bool = True, session_history: List[Dict[str, Any]] = None,
               enable_rag_judgment: bool = None, ) -> List[str]:
        """
        从知识库中检索与查询最相关的上下文信息。
        - query: 用户查询文本
...
class ExternalRAGService:
    ...
    def _should_use_rag(self, user_input: str, session_history: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        """
        prompt = """You are a RAG retrieval judgment assistant. Based on user input and conversation context, determine whether relevant information needs to be retrieved from the external knowledge base.

Judgment criteria:
- If the question involves specific domain knowledge, technical documentation, product information, FAQs, etc., retrieval should be performed
- If it is simple casual chat, emotional exchange, or general knowledge, no retrieval needed
- If the context clearly points to a specific domain question, retrieval should be performed
        ...
        try:
            ...
            if session_history:
                ...
                result = single_agent.chat(
                    ...
                    input_text=f"Current user input: {user_input}",
                    json_response=True,
                    temperature=0.1,
            ...
            if result["success"]:
                ...
                return {
                    "use_rag": True,
                    "confidence": 0.0,
                    "reason": result.get("error", lang("Judgment failed, defaulting to retrieve"))
                }
        except Exception as e:
            return {
                "use_rag": True,
                "confidence": 0.0,
                "reason": lang(f"Judgment exception: {str(e)}, defaulting to retrieve")
            }

    def search(self, query: str, score: float = 0.2, scope: str = "session", full_text: bool = True,
               enable_rag_judgment: bool = None, session_history: List[Dict[str, Any]] = None) -> List[str]:
        """
