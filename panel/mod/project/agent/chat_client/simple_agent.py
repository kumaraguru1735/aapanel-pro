...
class SimpleAgent:
    ...
    def _get_model_provider(self, model_name: str) -> str:
        ...
        返回: qwen, doubao, gpt, claude, gemini, deepseek, kimi, glm, hunyuan, ernie, other
        """
    ...
    def chat(self, user_input: Union[str, List[Dict[str, Any]]]) -> Generator[Dict[str, Any], None, None]:
        ...
        try:
            ...
            yield {"type": "error", "data": lang("API key error or invalid, please check if key is correct")}
        except openai.RateLimitError as e:
            yield {"type": "error", "data": lang("Rate limit exceeded, please try again later or increase quota: {}").format(e)}
        except openai.APIConnectionError as e:
            yield {"type": "error", "data": lang(f"Cannot connect to API server ({self.base_url}), please check network or address: {e}")}
